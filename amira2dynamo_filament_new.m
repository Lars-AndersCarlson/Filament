function table_out = amira2dynamo_filament_new(points,point_IDs,offsetx,offsety,offsetz,nmpix,tomo_nr,offset_reg_nr,template_fil,model_path,cropdist,crop_dphi)
%function table_out = amira2dynamo_filament_new(points,point_IDs,offsetx,offsety,offsetz,nmpix,tomo_nr,offset_reg_nr,template_fil,model_path,cropdist,crop_dphi)
%
...
%
%NOTE: This script version is modified to work with the new Amira version
%The changes are that the pixel size should be given in nm, and the
%tomogram dimension in y is no longer needed as an input.
%
...
%offsetz        (third value)
%nmpix          pixel size in Angstrom of the unbinned tomogram that will be
%               used by Dynamo for cropping. Unit: nm.
...
%
%usage example:
% table_out=amira2dynamo_filament_new(points,pointIDs,274.624,-274.624,4393.98,1.7164/4,10,0,'filaments_1/tomograms/volume_10/models/template/template.omd','filaments_1/tomograms/volume_10/models/',10,60);
...
offsetx=offsetx/nmpix;%in pixels
offsety=offsety/nmpix;%in pixels
offsetz=offsetz/nmpix;%in pixels

newpoints=zeros(size(points,1),3);%these transformations go from Amira to 3dmod/dynamo coordinate system (i.e. to pixels)
newpoints(:,1)=points(:,1)/nmpix-offsetx;
newpoints(:,2)=points(:,2)/nmpix-offsety;
newpoints(:,3)=points(:,3)/nmpix-offsetz;
